...
<head>
    ...
    window.onload = function(){  // window.onload vykonaju sa funkcie pri nacitani html DOM 
        js_print("Javascript", "window.onload", "Called");
        js_print("Javascript", "python_property", python_property);
        js_print("Javascript", "navigator.userAgent", navigator.userAgent); // ohladom browsera vypise info navigator.useragent
...
def main():
    check_versions()
    sys.excepthook = cef.ExceptHook  # To shutdown all CEF processes on error || ODCHYTAVANIE EXCEPTIONS
    settings = {  # ROZNE NASTAVENIA ZATIAL NETREBA RIESIT
    ...
    cef.Initialize(settings=settings)  # INICIALIZACIA CEF TOTO MUSIME VZDY TAK ISTO AJ UKONCIT CEF POMOCOU SHUTDOWN
    # set_global_handler()
...
def check_versions():
    ...
    assert cef.__version__ >= "57.0", "CEF Python v57.0+ required to run this"  # assert je nieco ako podmienka ak sa nieco vyhodnoti dobre program pokracuje dalej
...
# LOAD HANDLER PRI NACITANI DISPLAY HANDLER EVENTY KTORE SA DAJU ZOBRAZIT
